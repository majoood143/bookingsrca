...
use App\Filament\Resources\PromoCodeResource\RelationManagers\BookingsRelationManager;
...
class PromoCodeResource extends Resource
{
    ...
    {
        return parent::canAccess() && (bool) BookingSetting::get('module_promo_codes_enabled', true);
    }
            ...
            Section::make(__('promo.section_discount'))
                ...
                ->schema([
                    ...
                    TextInput::make('discount_value')
                        ...
                        ->step(0.01)
                        ->suffix(fn(Get $get): string => $get('discount_type') === 'percentage' ? '%' : 'OMR'),
                ]),
            ...
            Section::make(__('promo.section_scope'))
                ->schema([
                    Select::make('event_id')
                        ->label(__('promo.field_event'))
                        ->helperText(__('promo.field_event_helper'))
                        ->nullable()
        ...
        return $table
            ->columns([
                ...
                TextColumn::make('event.title')
                    ->label(__('promo.col_event'))
                    ->getStateUsing(fn(PromoCode $record): string => $record->event
                        ? $record->event->getTranslation('title', app()->getLocale())
                        : __('promo.all_events'))
                    ->badge()
                    ->color(fn(PromoCode $record) => $record->event_id ? 'info' : 'gray'),
            ...
            ->filters([
                ...
                SelectFilter::make('event_id')
                    ->label(__('promo.filter_event'))
                    ->options(
                        Event::all()->mapWithKeys(fn(Event $e) => [
