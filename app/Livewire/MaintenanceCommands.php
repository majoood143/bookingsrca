...
class MaintenanceCommands extends Component
{
    ...
    public function projectRefresh()
    {
        $this->runCommand('project:refresh');
    }
    ...
    public function deleteFailedJobs()
    {
        try {
            ...
            $this->output = "🗑️ Deleted {$count} record(s) from failed_jobs table.";
            $this->addToHistory('delete_failed_jobs', 'success', 0);
            $this->dispatch('notify', title: 'Success', message: 'Failed jobs deleted', status: 'success');
        } catch (Exception $e) {
            $this->output = "❌ Error deleting failed_jobs records: " . $e->getMessage();
            $this->addToHistory('delete_failed_jobs', 'error', 0);
        }
    }

    public function deleteMailRecords()
    {
        try {
            $count = DB::table('mails')->count();
            DB::table('mails')->delete();
            $this->output = "🗑️ Deleted {$count} record(s) from mails table.";
            $this->addToHistory('delete_mails', 'success', 0);
            $this->dispatch('notify', title: 'Success', message: 'Mail records deleted', status: 'success');
        } catch (Exception $e) {
            $this->output = "❌ Error deleting mails records: " . $e->getMessage();
            $this->addToHistory('delete_mails', 'error', 0);
        }
    }

    public function deleteEmailLogRecords()
    {
        try {
            $count = DB::table('filament_email_log')->count();
            DB::table('filament_email_log')->delete();
            $this->output = "🗑️ Deleted {$count} record(s) from filament_email_log table.";
            $this->addToHistory('delete_filament_email_log', 'success', 0);
            $this->dispatch('notify', title: 'Success', message: 'Email log records deleted', status: 'success');
        } catch (Exception $e) {
            $this->output = "❌ Error deleting filament_email_log records: " . $e->getMessage();
            $this->addToHistory('delete_filament_email_log', 'error', 0);
        }
    }
    ...
    public function render()
    {
        return view('livewire.maintenance-commands', [
            ...
            'mailsCount'     => $this->getTableCount('mails'),
            'emailLogCount'  => $this->getTableCount('filament_email_log'),
            'failedJobsCount' => $this->getTableCount('failed_jobs'),
        ]);
    }
}
