...
<div class="space-y-6">
    ...
    <x-filament::section>
        <x-slot name="heading">
            <div class="flex items-center gap-3">
                <x-filament::icon icon="heroicon-s-envelope" class="w-5 h-5 text-orange-500" />
                Mail & Data Cleanup
            </div>
        </x-slot>
        <x-slot name="description">
                ...
                <div class="flex flex-col justify-between gap-3 p-4 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                    <div>
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-semibold text-gray-900 dark:text-white">{{ $target['label'] }}</span>
                            <x-filament::badge :color="$target['count'] > 0 ? 'warning' : 'gray'">
                                {{ number_format($target['count']) }}
                            </x-filament::badge>
                        </div>
                        <code class="font-mono text-gray-400 dark:text-gray-500" style="font-size:10px">{{ $target['table'] }}</code>
                    </div>

                    <x-filament::button
                        x-on:click="
                            if (confirm('Permanently delete all {{ number_format($target['count']) }} record(s) from the {{ $target['table'] }} table? This cannot be undone.')) {
                                $wire.call('{{ $target['method'] }}');
                            }
                        "
                        wire:loading.attr="disabled"
                        color="danger"
                        ...
                        :icon="'heroicon-s-trash'"
                    >
                        <span wire:loading.remove wire:target="{{ $target['method'] }}">Delete Records</span>
                        <span wire:loading wire:target="{{ $target['method'] }}" class="flex items-center gap-1">
                            <x-filament::loading-indicator class="w-3 h-3" />
                            Deleting…
                        </span>
                    </x-filament::button>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        @foreach (array_reverse($commandHistory) as $entry)
                            <tr>
                                <td class="px-3 py-2 font-mono text-xs text-gray-700 dark:text-gray-300">{{ $entry['command'] }}</td>
                                <td class="px-3 py-2 text-xs text-gray-500 dark:text-gray-400">{{ $entry['timestamp'] }}</td>
                                <td class="px-3 py-2 text-xs text-gray-500 dark:text-gray-400">{{ $entry['execution_time'] }}s</td>
                                <td class="px-3 py-2">
                                    <x-filament::badge
